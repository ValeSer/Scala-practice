...
class FizzBuzzTest extends AnyFunSuite {
  ...
  test("prints a list including FizzBuzz if divisible by 3 and 5") {
    assert(FizzBuzz.generate(15) === "1, 2, Fizz, 4, Buzz, Fizz, 7, 8, Fizz, Buzz, 11, Fizz, 13, 14, FizzBuzz")
  }
  test("prints a list including Fizz if divisible by 3") {
    assert(FizzBuzz.generate(3) === "1, 2, Fizz")
  }
  test("prints a list including Buzz if divisible by 5") {
    assert(FizzBuzz.generate(5) === "1, 2, Fizz, 4, Buzz")
  }
  test("prints the number in a list if not divisible by 3, 5, or both") {
  ...
  test("prints empty string if negative number") {
    assert(FizzBuzz.generate(-1) === "")
  }
}
